...
{
	// Check if we have reached the 8th row and 9th column (0 indexed matrix) , returning true to avoid backtracking
	...
	// Check if the current position of grid contains value>0
	if (grid[row][col] > 0)
	...
	for (int num = 1; num <= N; num++)
	{
		...
		// Assigning the num in  the current (row,col) of the grid & asuming that assigned position is correct
			grid[row][col] = num;
		...
		// Removing the assigned num as our assumption was wrong
		grid[row][col] = 0;
	}
	return false;
}
